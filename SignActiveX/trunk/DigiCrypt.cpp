// This part of code is modified version of digicrypt functions from digidoclib
// the main purpose of this file is to show dialog bos while user has more than one certificate in its certificate store.
// 
#include "stdafx.h"
#include "DigiCrypt.h"
#include <stdio.h>
...
static BOOL            RunDlg_FillItem(PCCERT_CONTEXT  pCertContext, char *psRes);
...
hStore = CertOpenStore(CERT_STORE_PROV_SYSTEM,0,(HCRYPTPROV)NULL, CERT_SYSTEM_STORE_CURRENT_USER | CERT_STORE_READONLY_FLAG | CERT_STORE_OPEN_EXISTING_FLAG,L"MY");

//hStore = CertOpenStore(CERT_STORE_PROV_SYSTEM,0,(HCRYPTPROV)NULL,CERT_SYSTEM_STORE_CURRENT_USER,L"MY");

//hStore = CertOpenStore(CERT_STORE_PROV_SYSTEM,0,(HCRYPTPROV)NULL,CERT_SYSTEM_STORE_CURRENT_USER  | CERT_STORE_BACKUP_RESTORE_FLAG   |
//CERT_STORE_READONLY_FLAG   | CERT_STORE_OPEN_EXISTING_FLAG,L"MY");
return(hStore);
}
...
if (fRes == TRUE)
  {
  poKeyInfo = (CRYPT_KEY_PROV_INFO *)malloc(pcbData);
  if (poKeyInfo != NULL)
    {
...
if (fRes == TRUE)
  ...
  lstrcpy(oG_sKeyContainerName,(char *) pbData);
...
if (iListItems > 1)
  {
  LOG("StartDialog");
  if(oG_selectedCert<0)
    iRes = RunDialogUnit(psListItems, 450,300);
  else
	iRes=oG_selectedCert;

  oG_selectedCert=iRes;
  if (iRes < 0)
...
if (FileTimeToSystemTime(&pCertInfo->NotBefore,&oT) == TRUE)
  {
  sprintf(sTemp,"%02d/%02d/%02d   \t",oT.wYear,oT.wMonth,oT.wDay);
  lstrcat(psRes,sTemp);
  }
else
  fRes = FALSE;
