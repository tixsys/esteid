 ...
 * @param selectSignCertificate callback function for selecting the correct signing
 *        certificate. If none of the certificates suit for signing, this callback
 *        function should return <code>NULL</code>. This function is always called,
 *        when there are at least 2 certificates available.
 * @param getPin callback function that returns PIN code for the selected signing
 *        certificate. If PIN code is not needed this function is never called.
 * @throws SignException exception is thrown if the provided PKCS #11 driver
 *         loading failed.
 */
digidoc::PKCS11Signer::PKCS11Signer(const std::string& driver,
 ...
 : driver(driver)
 , selectSignCertificate(selectSignCertificate)
 , getPin(getPin)
 , ctx(NULL)
 ...
 , numberOfSlots(0)
{
    DEBUG("PKCS11Signer(driver = '%s', selectSignCertificate = 0x%X, getPin = 0x%X)",
            driver.c_str(), (unsigned int)selectSignCertificate, (unsigned int)getPin)
...
X509* digidoc::PKCS11Signer::getCert() throw(SignException)
{
    ...
    // If there are more than 1 certificate, let the application select
    // the signing certificate, otherwise use the the only one found.
    X509* selectedCert = certificates[0].cert;
    if(certificates.size() > 1)
    {
        if(selectSignCertificate == NULL)
        {
            THROW_SIGNEXCEPTION("Callback function selectSignCertificate is NULL.")
        }
        selectedCert = selectSignCertificate(certificates);
    }
...
{
    INFO("sign(digest = {type=%d,digest=%s,length=%d}, signature={signature=0x%X,length=%d})",
            digest.type, (char*)digest.digest, digest.length, (unsigned int)signature.signature, signature.length)
