...
public class SAXDigiDocFactory extends DefaultHandler implements DigiDocFactory {
    ...
    /**
     * This mode means collect SAX events into xml data and is used to collect all <DataFile>, <SignedInfo> and <SignedProperties> content. Also servers as level of embedded
     * DigiDoc files. Initially it should be 0. If we start collecting data then it's 1 and if we find another SignedDoc inside a DataFile then it will be incremented in order to
     * know which is the correct </DataFile> tag to leave the collect mode
     */
    ...
    public boolean verifyCertificate(X509Certificate cert) throws DigiDocException {
        ...
        if (m_rootCerts != null && !m_rootCerts.isEmpty()) {
            try {
                X509Certificate rCert = (X509Certificate) m_rootCerts.get(cert.getIssuerDN().getName());
                if (rCert != null) {
     ...
     * 
     * @param digiDocStream
     *            opened stream with DigiDoc data The user must open and close it.
     * @return signed document object if successfully parsed
     */
     ...
     * 
     * @param sdoc
     *            SignedDoc to add this signature to
     * @param sigStream
     *            opened stream with Signature data The user must open and close it.
     * @return signed document object if successfully parsed
     */
    public Signature readSignature(SignedDoc sdoc, InputStream sigStream) throws DigiDocException {
        ...
        if (m_doc.getLastSignature() == null)
            throw new DigiDocException(DigiDocException.ERR_DIGIDOC_FORMAT, "This document is not in Signature format", null);
    ...
    /*
     * private void debugWriteFile(String name, String data) { try { String str = "C:\\veiko\\JDigiTest1\\" + name; System.out.println("Writing debug file: " + str);
     * FileOutputStream fos = new FileOutputStream(str); fos.write(data.getBytes("UTF-8")); fos.close(); } catch(Exception ex) { System.out.println("Error: " + ex);
    ...
    public void startElement(String namespaceURI, String lName, String qName, Attributes attrs) throws SAXDigiDocException {
        ...
        m_tags.push(qName);
        if (qName.equals("SigningTime") || qName.equals("IssuerSerial") || qName.equals("X509SerialNumber") || qName.equals("X509IssuerName") || qName.equals("ClaimedRole") || qName.equals("City")
                || qName.equals("StateOrProvince") || qName.equals("CountryName") || qName.equals("PostalCode") || qName.equals("SignatureValue") || qName.equals("DigestValue")
                ||
                // qName.equals("EncapsulatedX509Certificate") ||
                qName.equals("IssuerSerial") || qName.equals("ResponderID") || qName.equals("X509SerialNumber") || qName.equals("ProducedAt") || qName.equals("EncapsulatedTimeStamp")
        ...
        if (m_nCollectMode == 0) {
            ...
            // <Signature>
            /*
             * if (qName.equals("Signature")) { String Id = attrs.getValue("Id"); try { System.out.println("Creating signature: " + Id); Signature sig = new Signature(m_doc); if(Id
             * != null) sig.setId(Id); m_doc.addSignature(sig); } catch (DigiDocException ex) { SAXDigiDocException.handleException(ex); } }
             */
            // <KeyInfo>
    ...
    public void endElement(String namespaceURI, String sName, String qName) throws SAXException {
        ...
        if (qName.equals("SignedProperties")) {
            ...
            try {
                ...
                if (cid != null) {
                    /*
                     * System.out.println("CID: " + cid.getId() + " serial: " + cid.getSerial() + " alg: " + cid.getDigestAlgorithm() + " diglen: " + ((cid.getDigestValue() ==
                     * null) ? 0 : cid.getDigestValue().length));
                     */
                    if (cid.getId() != null)
        ...
        // </IssuerSerial>
        if (qName.equals("IssuerSerial") && !m_doc.getVersion().equals(SignedDoc.VERSION_1_3) && !m_doc.getVersion().equals(SignedDoc.VERSION_1_4)) {
            try {
                Signature sig = getLastSignature();
        ...
        // </X509SerialNumber>
        if (qName.equals("X509SerialNumber") && (m_doc.getVersion().equals(SignedDoc.VERSION_1_3) || m_doc.getVersion().equals(SignedDoc.VERSION_1_4))) {
            try {
                Signature sig = getLastSignature();
                CertID cid = sig.getLastCertId();
                if (cid != null)
        ...
        // </X509IssuerName>
        if (qName.equals("X509IssuerName") && (m_doc.getVersion().equals(SignedDoc.VERSION_1_3) || m_doc.getVersion().equals(SignedDoc.VERSION_1_4))) {
            try {
                Signature sig = getLastSignature();
                CertID cid = sig.getLastCertId();
