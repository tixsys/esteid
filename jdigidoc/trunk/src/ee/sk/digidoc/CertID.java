...
public class CertID implements Serializable
{
     ...
     * default values for id and cert 
     * @param sig Signature object
     * @param cert certificate for creating this ref data
     * @param type CertID type: signer, responder or tsa or ca
     * @throws DigiDocException for validation errors
     */
        ...
        throws DigiDocException
    {
        ...
        try {
        	...
        	if((m_signature.getSignedDoc().getFormat().equals(SignedDoc.FORMAT_BDOC) && (m_type == CertID.CERTID_TYPE_CA)) ||
			...
			bos.write(ConvertUtils.str2data("\n<CertDigest>\n<DigestMethod Algorithm=\""));
        		...
        		if(m_signature.getSignedDoc().getVersion().equals(SignedDoc.VERSION_1_3) ||
        			...
        			//IS FIX emailAddress
        			if (m_signature.getSignedDoc().getFormat().equals(SignedDoc.FORMAT_BDOC)){
        				if (m_issuer.indexOf("E=") != -1){
        					m_issuer = m_issuer.replace("E=", "emailAddress=");
        				}
        				if (m_issuer.indexOf("OID.1.2.840.113549.1.9.1=") != -1) {
        					m_issuer = m_issuer.replace("OID.1.2.840.113549.1.9.1=", "emailAddress=");
        				}
        			}
