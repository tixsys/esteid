...
public class IAIKNotaryFactory implements NotaryFactory 
{
    ...
    {    	
        return getConfirmation(nonce, 
        	signersCert, getCACert(SignedDoc.getCommonName(signersCert.getIssuerX500Principal().getName("RFC1779"))), notId);
    }
        ...
        throws DigiDocException 
    {
        try {
        	...
        	// create the request
        	X509Certificate caCert = getCACert(SignedDoc.getCommonName(cert.getIssuerX500Principal().getName("RFC1779")));
            ...
            if(p12file != null && p12paswd != null) {
				...
				for(int i = 0; (certs != null) && (i < certs.length); i++) {
					...
					if(m_logger.isDebugEnabled()) {
                    	...
                    	m_logger.debug("Cert " + i + " issuer: " + cert.getIssuerX500Principal().getName("RFC1779"));                    
