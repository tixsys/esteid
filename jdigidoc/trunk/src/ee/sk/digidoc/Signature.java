...
public class Signature implements Serializable
{
     ...
     * @param cval new object to be addedsetid
     */
        ...
        throws DigiDocException
    {
        ...
        if (m_sigDoc.getFormat().equals(SignedDoc.FORMAT_BDOC)){
        	ddocFac = ConfigManager.instance().getBDigiDocFactory();
        }
        else {
      ...
      //IS FIX CACERT
        if (SignedDoc.FORMAT_BDOC.equals(m_sigDoc.getFormat())){
        	CertValue cval = new CertValue();
            cval.setType(CertValue.CERTVAL_TYPE_CA);
            cval.setCert(caCert);
            addCertValue(cval);
            cval.setId(m_id + "-CA_CERT");
          //IS FIX CACERT
    ...
    public ArrayList verify(SignedDoc sdoc, boolean checkDate, boolean demandConfirmation)
    {
        ...
        try {
        	...
        	// kalev <2008 aprill> use BDigiDocFactory
        	if(SignedDoc.FORMAT_BDOC.equals(sdoc.getFormat())){
        		digFac = ConfigManager.instance().getBDigiDocFactory();
        	}else{
        		digFac = ConfigManager.instance().getDigiDocFactory();
        	}
        //L Kalev <2008 aprill> BDOCiga seotud muudatused xml-is 1
    	...
    	if(m_origContent == null) {
        	...
        	try {
            	...
            	if((m_sigDoc.getVersion().equals(SignedDoc.VERSION_1_3)) || 
            		//A Inga <2008 aprill> BDOCiga seotud muudatused xml-is 1
            	//IS fix algus
        			(m_sigDoc.getFormat().equals(SignedDoc.FORMAT_BDOC))){
            	//IS fix lopp
        			//L Inga <2008 aprill> BDOCiga seotud muudatused xml-is 1
