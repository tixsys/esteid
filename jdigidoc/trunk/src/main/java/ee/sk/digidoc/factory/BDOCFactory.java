...
public class BDOCFactory extends DefaultHandler implements
		DigiDocFactory {
	...
	public SignedDoc readSignedDoc(String fileName) throws DigiDocException {
		...
		ZipEntry mimeEntry = null;
		ZipEntry sigFileEntry = null;
		ZipEntry manifestEntry = null;
		InputStream mimeInput = null;
		InputStream sigFileInput = null;
		...
		try {
			...
			if(m_logger.isDebugEnabled())
				m_logger.debug("readBDoc: " + zipFile.getName());
			
			// read mimetype file
			//TODO:  kui faili pole zipis
			
			if (zipFile.getEntry(SignedDoc.MIMET_FILE_NAME) != null){
				mimeEntry = zipFile.getEntry(SignedDoc.MIMET_FILE_NAME);
				mimeInput = zipFile.getInputStream(mimeEntry);
				readMimeTypeFile(mimeInput);
				if (mimeInput != null) mimeInput.close();
			}
	        
			if (zipFile.getEntry(SignedDoc.MANIF_FILE_NAME) != null){
				manifestEntry = zipFile.getEntry(SignedDoc.MANIF_FILE_NAME);
				manifestInput = zipFile.getInputStream(manifestEntry);
				saxParser.parse(manifestInput, handler);
				if (manifestInput != null) manifestInput.close();
			}
	        
			// read signature1.xml file
			sigFileEntry = zipFile.getEntry(SignedDoc.SIG_FILE_NAME);
			sigFileInput = zipFile.getInputStream(sigFileEntry);
			saxParser.parse(sigFileInput, handler);
			if (sigFileInput != null) sigFileInput.close();
	...
	private void readMimeTypeFile(InputStream mimeInput) throws DigiDocException{
		BufferedReader br = null;
		String line;
		String version = "";
		try {
			br = new BufferedReader(new InputStreamReader(mimeInput));
			line = br.readLine();
			if (SignedDoc.MIMET_FILE_CONTENT.equals(line.substring(0, SignedDoc.MIMET_FILE_CONTENT.length()))){
				version = line.substring(SignedDoc.MIMET_FILE_CONTENT.length()+1);
				if (!version.equals(""))
					m_doc = new SignedDoc(SignedDoc.FORMAT_BDOC, version);
			} else {
				throw new DigiDocException(
						DigiDocException.ERR_MIMETYPE_FILE,
						"Mimetype file har wrong content",
						null);
			}
	...
	private SignedDoc readBDoc(String fileName) 
		throws DigiDocException {
		// Use an instance of ourselves as the SAX event handler
		BDOCFactory handler = this;
		// Use the default (non-validating) parser
		...
		try {
			...
			if(m_logger.isDebugEnabled())
				m_logger.debug("readBalticDoc: " + jarFile.getName());
			
			JarEntry entry = jarFile.getJarEntry("META-INF\\signature1.xml");
		...
		throws SAXDigiDocException 
	{
		...
		if (qName.equals("SigningTime") ||
		   ...
		   qName.equals("manifest:file-entry") )
		...
		if(qName.equals("manifest:file-entry")) {
			...
			for (int i = 0; i < attrs.getLength(); i++) {
				...
				if (key.equals("manifest:media-type")) {
					mediaType = attrs.getValue(i);
				}
				if (key.equals("manifest:full-path")) {
					fullPath = attrs.getValue(i);
				}
			}
				//String mediaType  = attrs.getValue("media-type");
				//String fullPath  = attrs.getValue("full-path");
			if (m_logger.isDebugEnabled())
				m_logger.debug("Reading manifest.xml : mediaType: " + mediaType + "  fullPath : " + fullPath);
			/*if (m_nCollectMode == 0) {
				try {
					DataFile df = new DataFile("", DataFile.CONTENT_EMBEDDED, fullPath, mediaType, m_doc);
			...
			if (m_nCollectMode == 0) {
				try {
					...
					if(df.schouldUseTempFile()) {
						...
						if(m_logger.isDebugEnabled())
							m_logger.debug("DF: " + Id + " size: " + df.getSize() + 
		...
		//if (m_nCollectMode == 0) {
			...
			if (qName.equals("EncapsulatedOCSPValue")) {
				try {
					...
					System.out.println("EncapsulatedOCSPValue: " + (m_sbCollectItem != null ? m_sbCollectItem.toString() : "NULL"));
