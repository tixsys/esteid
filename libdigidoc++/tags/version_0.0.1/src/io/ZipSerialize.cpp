...
void digidoc::ZipSerialize::extractCurrentFile(unzFile zipFile, const std::string& directory) throw(IOException)
{
    ...
    // Create sub directory. Check for directory in ZIP file.
    if(fileName[fileName.length() - 1] == '/')
    {
        std::string subDirectory = Util::File::path(directory, fileName);
        DEBUG("Creating directory: '%s'", subDirectory.c_str());
...
void digidoc::ZipSerialize::save() throw(IOException)
{
    ...
    for(std::vector<FileEntry>::const_iterator iter = filesAdded.begin(); iter != filesAdded.end(); iter++)
    {
        ...
        FILE* pFile = fopen(iter->path.c_str(), "rb");
        if(pFile == NULL)
        {
