...
digidoc::dsig::SignatureType* digidoc::Signature::parseSignature(const std::string& path)
{
    ...
    catch(const xml_schema::Parsing& p)
    {
...
void digidoc::Signature::setSigningCertificate(const X509Cert& x509)
{
    ...
    //DEBUG("Signing cert, serial: %ld", x509.getCertSerial());
    //DEBUG("Signing cert, data: \n%s",pemX509.encode().c_str());
...
/**
 *
 * @return SignatureValue's contents
 */
std::string digidoc::Signature::getBase64SignatureValue() const
{
    dsig::SignatureType::SignatureValueType syntValue = signature->signatureValue();
    return syntValue.encode();
}
 ...
 *
 * @param calc implementation of digest calculator.
 * @param node node to be digested.
 * @return returns digest of the XMl node.
 */
std::vector<unsigned char> digidoc::Signature::calcDigestOnNode(Digest* calc, xercesc::DOMNode* node)
{
    std::vector<unsigned char> c14n = c14nNode(node);
    DEBUG("c14n = '%s'", reinterpret_cast<char*>(&c14n[0]) );

    calc->update(&c14n[0], c14n.size());
