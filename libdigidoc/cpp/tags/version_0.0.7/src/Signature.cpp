		...
		const std::string& tagName) const throw(SignatureException)
{
	try
	{
	    ...
	    std::auto_ptr<XMLCh> tagNs(xercesc::XMLString::transcode(ns.c_str()));
	    std::auto_ptr<XMLCh> tag(xercesc::XMLString::transcode(tagName.c_str()));
        xercesc::DOMNodeList* nodeList = dom->getElementsByTagNameNS(tagNs.get(), tag.get());
 ...
 std::auto_ptr<xercesc::DOMDocument> digidoc::Signature::createDom() const throw(IOException)
{
    ...
    {
        std::auto_ptr<char> message(xercesc::XMLString::transcode(e.getMessage()));
        std::string msg(message.get());
        THROW_IOEXCEPTION("Failed to parse signature XML: %s", msg.c_str());
    }
    catch(const xercesc::DOMException& e)
    {
        std::auto_ptr<char> message(xercesc::XMLString::transcode(e.msg));
        std::string msg(message.get());
        THROW_IOEXCEPTION("Failed to parse signature XML: %s", msg.c_str());
    }
    catch(...)
