...
void digidoc::SignatureTM::sign(Signer* signer) throw(SignatureException)
{
    ...
    setCompleteRevocationRefs(ocspCert_.getIssuerName(), calc->getUri(), nonce, producedAt);
 ...
 */
void digidoc::SignatureTM::setCompleteRevocationRefs(const std::string& responderName, const std::string& nonceURI,
        const std::vector<unsigned char>& nonce, const struct tm& producedAt )
{
    dsig::DigestMethodType method(xml_schema::Uri(nonceURI.c_str()));
