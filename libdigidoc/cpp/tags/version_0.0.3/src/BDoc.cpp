...
void digidoc::initialize()
{
	...
	// Initialize Apache XML Security librari.
...
{
	// Terminate Apache XML Security librari.
    ...
    try
    {
        ...
        for(manifest::Manifest::File_entrySequence::const_iterator iter = manifest->file_entry().begin(); iter != manifest->file_entry().end(); iter++)
        {
            ...
            if(iter->full_path().find_first_of("META-INF/") == 0)
            {
                ...
                {
                    signatures.push_back(new SignatureBES(util::File::path(util::File::path(path, "META-INF"), fileName), *this));
                }
                else
                {
 ...
 * @param profile signature profile (e.g. BES).
 * @throws BDocException
 */
// TODO: the methods called here can throw other exceptions. Handle these or throw?
void digidoc::BDoc::sign(Signer* signer, Signature::Type profile) throw(BDocException)
{
	...
	// Calculate SHA1 digests for the documents and add these to the signature reference.
    for(std::vector<Document>::iterator iter = documents.begin(); iter != documents.end(); iter++)
    {
	    ...
	    SHA1Digest calc;
        signature->addReference(uri, NID_sha1, iter->calcDigest(calc));
    }
